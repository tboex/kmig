[![](https://github.com/tboex/kmig/blob/main/frontend/static/images/github.png)](https://kmig.io/)

![React](https://img.shields.io/badge/React-087ea4?style=for-the-badge&logo=React&logoColor=white)
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![Fontawesome](https://img.shields.io/badge/fontawesome-538DD7?style=for-the-badge&logo=fontawesome&logoColor=white)
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=Vite&logoColor=white)


# About
an over-engineered version of the Korean word game 끝말잇기

# Features
- simple, no sign-in needed game
- minimalistic design
- play with friends
- practice korean vocab
- each word will give details like english translation, pronunciation, etc

# Credits
All me @timboex

I mainly do backend development, so I was heavily inspired by [Monkeytype](www.monkeytype.com)'s website and used it as a learning place for how to do React/Vite
